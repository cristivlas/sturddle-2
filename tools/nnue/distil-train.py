...
'''
*****************************************************************************
This is the "teacher" model for online distillation training.

*****************************************************************************
'''
def make_teacher_model(args):
    def transform_encoded_data_tensor(encoded_data_tensor):
        board_data_tensor = encoded_data_tensor[:, :768]
        turn_indicator_tensor = encoded_data_tensor[:, 768:]
        reshaped_board_data_tensor = tf.reshape(board_data_tensor, [-1, 8, 8, 12])
        transformed_data_tensor = tf.transpose(reshaped_board_data_tensor, perm=[0, 2, 1, 3])
        turn_indicator_matrix = tf.reshape(turn_indicator_tensor, [-1, 1, 1, 1])
        turn_indicator_matrix = tf.tile(turn_indicator_matrix, [1, 8, 8, 1])
        return tf.concat([transformed_data_tensor, turn_indicator_matrix], axis=-1)

    inputs = Input(shape=(769,))
    transformed = Lambda(transform_encoded_data_tensor)(inputs)

    # attention = MultiHeadAttention(num_heads=64, key_dim=7)(transformed, transformed, transformed)
    # conv1 = Conv2D(32, kernel_size=(3, 3), activation='relu', padding='same')(attention)
    conv1 = Conv2D(16, kernel_size=(3, 3), activation='relu', padding='same')(transformed)
    conv2 = Conv2D(32, kernel_size=(5, 5), activation='relu', padding='same')(conv1)
    # conv3 = Conv2D(64,kernel_size=(7, 7), activation='relu', padding='same')(conv2)
    # flat = Flatten()(conv3)

    flat = Flatten()(Attention()([conv2, conv2]))

    dense1 = Dense(4096, activation='relu')(flat)
    dense2 = Dense(1024, activation='relu')(dense1)
    dense3 = Dense(256, activation='relu')(dense2)
    dense4 = Dense(64, activation='relu')(dense3)
    outputs = Dense(1, dtype=tf.float32)(dense4)
    ...
    model.compile(optimizer=optimizer, loss=loss_function(args))
...
def main(args):
    ...
    def train_with_distillation(
        ...
        for epoch in range(epochs):
            ...
            for batch, (inputs, labels) in enumerate(train_dataset):
                ...
                progress_values = [ ('student', student_loss), ('teacher', teacher_loss), ('distillation', distillation_loss) ]
    ...
    teacher_model_path = f'{args.model_path}-teacher' if args.model_path else None
