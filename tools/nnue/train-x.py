...
def make_model(args, strategy):
    ...
    def compute_bucket_id(features, num_buckets):
    ...
    class BucketedDense(tf.keras.layers.Layer):
        """Equivalent to BucketShift -> Dense, but skips the all-zero buckets.

        The kernel is a single (num_buckets * features, units) tensor, laid out
        bucket-major to match a plain Dense over the sparse bucket-shifted input
        and the C++ inference convention (base = bucket * ACTIVE_INPUTS). Instead
        of materializing the (batch, num_buckets * features) sparse vector and
        multiplying mostly zeros, each row gathers only its own bucket's weight
        block and does a (features x units) matmul.
        """
        def __init__(self, num_buckets, units, activation=None,
                     kernel_initializer=None, kernel_constraint=None,
                     bias_constraint=None, trainable=True, **kwargs):
            super(BucketedDense, self).__init__(trainable=trainable, **kwargs)
            ...
            self._on_gpu = bool(tf.config.list_physical_devices('GPU'))

        def build(self, input_shape):
            self.features = int(input_shape[-1])
            self.kernel = self.add_weight(
                name='kernel',
                shape=(self.num_buckets * self.features, self.units),
        ...
        def call(self, features):
            bucket_id = compute_bucket_id(features, self.num_buckets)  # (batch,)

            if self._on_gpu:
                # GPU: dense tile+mask path. dynamic_partition/stitch stall the GPU
                # (data-dependent shapes), so multiply the bucket-sparse vector instead.
                mask = tf.one_hot(bucket_id, self.num_buckets, dtype=features.dtype)  # (batch, NB)
                sparse = tf.reshape(features[:, None, :] * mask[:, :, None], [-1, self.num_buckets * self.features])
                out = tf.matmul(sparse, self.kernel) + self.bias
            else:
                # CPU: split rows by bucket, matmul each group against its own weight block, stitch back.
                blocks = tf.reshape(self.kernel, [self.num_buckets, self.features, self.units])
                row_idx = tf.range(tf.shape(features)[0])
                part_rows = tf.dynamic_partition(features, bucket_id, self.num_buckets)
                part_idx = tf.dynamic_partition(row_idx, bucket_id, self.num_buckets)
                part_out = [tf.matmul(part_rows[b], blocks[b]) for b in range(self.num_buckets)]
                out = tf.dynamic_stitch(part_idx, part_out) + self.bias
