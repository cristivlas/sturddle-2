...
DEFINES = ['NATIVE_BUILD=1', 'NATIVE_UCI=1', 'NATIVE_BOOK=1', 'WITH_NNUE', 'SHARED_WEIGHTS', 'CALLBACK_PERIOD=8192', 'NO_ASSERT']
...
def build_windows(arch, version, build_stamp):
...
def build_linux(arch, version, build_stamp):
...
def main():
    ...
    if sys.platform.startswith('win'):
        exe = build_windows(args.arch, version, build_stamp)
    elif sys.platform.startswith('linux'):
        exe = build_linux(args.arch, version, build_stamp)
    else:
        sys.exit(f'Unsupported platform: {sys.platform}')

    for asset in ('weights.bin', 'book.bin'):
        src = REPO_ROOT / asset
        if src.exists():
